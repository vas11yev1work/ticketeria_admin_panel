        ...
        <v-container>
            <v-row>
                ...
                <v-col class="col-12" lg="9">
                    ...
                    <p>{{ gateway.gateway.id }}</p>
                    <!--<v-form ref="gateway" lazy-validation>
...
<script>
    export default {
        ...
        async mounted() {
            if(this.$store.getters.gateways.length === 0){
                await this.$store.dispatch('fetchGateways');
            }
        },
        computed: {
            gateways(){
                return this.$store.getters.gateways;
            },
            gateway(){
                const gateway = this.gateways.find(i => i.id === this.$route.params.id);
                return { gateway };
            }
        }
    }
